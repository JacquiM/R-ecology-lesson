...
you used in the previous chapter supports connecting to the widely-used open
...
In order to connect with the database and execute the correct queries, we need to install dplyr, RSQLite and devtools.

```
{r install, results='markup', purl=FALSE}
install.packages("dplyr")
```
```
install.packages("RSQLite")
{r installrsqlite, results='markup', purl=FALSE}
```
```
install.packages("devtools")
{r installdev, results='markup', purl=FALSE}
```
...
```{r downloads, results='markup', purl=FALSE}
dir.create("data", showWarnings = FALSE)
```
Open your browser and download the database from: https://ndownloader.figshare.com/files/2292171
Locate the downloaded database, copy it and paste it in the newly created "data" file. This is usually found in documents.
...
```{r explain, message=TRUE, purl=FALSE}
...
The taxa of a species, from the species dataset, can be joined to the year in which it was surveyed, from the survey dataset.
...
records, and join the two tables. For example, to extract all surveys for the
first plot, which has `plot_id` 1, we can do:
